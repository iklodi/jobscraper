    ...
    async with async_playwright() as p:
        ...
        for keyword in SEARCH_KEYWORDS_LIST:
            for location in SEARCH_LOCATIONS:
                query = urllib.parse.urlencode({'keywords': keyword, 'location': location})
                search_url = f"https://www.linkedin.com/jobs/search/?{query}"
                
                print(f"Searching for jobs: {keyword} in {location}")
                ...
                print(f"Finished scraping '{keyword}' in {location}. Added {new_jobs_count} new jobs.")
